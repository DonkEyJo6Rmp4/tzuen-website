...
公開 GitHub repository 前，必須將「原始工作素材」與「網站公開素材」完全分離：

1. `resource/assets/` 只保存於本機，放置原始高解析照片、證書掃描與可編輯原檔，並加入 `.gitignore`。
2. `public/images/` 只放置網站實際使用、已適度壓縮且確認可公開的圖片與 SVG。
3. `public/documents/` 只放置網站需公開的壓縮文件，不放名冊、身分證明、私人聯絡資料或未遮罩個資。
4. 可識別兒少的照片上線前，由基金會確認已取得符合此次網站公開用途的授權。
5. GitHub Pages 只部署 `npm run build` 產生的 `dist/`，不得將工作區根目錄直接當成發佈目錄。
6. `.gitignore` 只能阻止未來追蹤，不會清除舊 commit 中的素材。若原始素材曾經被 commit，必須在轉為 Public 前備份並重寫 Git 歷史。
